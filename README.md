...
Профиль **Telegram** применяет TCP/UDP desync к известным подсетям Telegram. Для звонков это даёт
отдельную попытку обработки UDP, но рабочим результатом мы считаем только то, что прошло проверку на
реальном устройстве и сети — до этого README не обещает гарантированную поддержку звонков.
